...
|7|How Is Gradle Used ?|<ins>Greate a build script</ins><br/>&ensp;- Kotlin(extension file : std : `*.kt` & `*.kts` for script) or Groovy DSL<br/><br/><ins>This defines tasks</ins><br/>&ensp;- build, clean, etc...<br/><br/><ins>Run the appropriate task</ins><br/>&ensp;- From the CLI or an IDE|build.gradle:<br/>`task 'hello' {`<br/>`	doLast {`<br/>`		println "Hello Gradle"`<br/>`	}`<br/>`}`<br/><br/>build.gradle.kts :<br/>`tasks.register("hello") {`<br/>`	doLast {`<br/>`		println("Hello Gradle")`<br/>`	}`<br/>`}`|
